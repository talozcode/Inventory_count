...
const { readRange, appendRow, updateRange, clearRange } = require('./sheets');
...
async function syncOneProductToOdooUpdate(internalRef, entries) {
  ...
  if (existingRow) {
    ...
    await updateRange(`${SHEET_ODOO_UPDATE}!A${existingRow}:F${existingRow}`, [[
      preserveDone, internalRef, product.productName, finalQty, status, finalNote,
    ]]);
  } else {
    await appendRow(`${SHEET_ODOO_UPDATE}!A:F`, [
...
async function submitCount(payload) {
  ...
  if (!lookup.found) {
    await appendRow(`${SHEET_UNKNOWN}!A:G`, [
